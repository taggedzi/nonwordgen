...
class IndonesianLanguagePlugin(LanguagePlugin):
    ...
    def build_dictionary(
        ...
        backends: list[DictionaryBackend] = [StaticWordSetDictionary(COMMON_INDONESIAN_WORDS)]
        ...
        if strictness in {Strictness.MEDIUM, Strictness.STRICT, Strictness.VERY_STRICT}:
            ...
            wordfreq_backend = WordfreqDictionary(real_word_min_zipf=threshold, language="id")
        ...
        if strictness in {Strictness.STRICT, Strictness.VERY_STRICT}:
            wordset_backend = WordsetDictionary(language="id")
            if getattr(wordset_backend, "available", False):
                backends.append(wordset_backend)
            else:
                logger.warning(
                    "wordset backend unavailable for Indonesian; %s strictness is degraded.",
                    strictness.value,
                )
